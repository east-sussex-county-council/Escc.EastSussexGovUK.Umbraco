    ...
    public class JobsDataFromExamine : IJobsDataProvider
    {
        ...
        public Task<List<Job>> ReadJobs(JobSearchQuery query)
        {
            ...
            examineQuery.Field("reference", query.JobReference ?? String.Empty)
                        ...
                        .And().GroupedOr(new[] { "salaryRange" }, query.SalaryRanges.ToArray())
                        .And().GroupedOr(new[] { "workPattern" }, query.WorkPatterns.ToArray()).Compile();
            ...
            foreach (var result in results)
            {
                var job = new Job()
                {
                    ...
                    Salary = result.Fields.ContainsKey("salary") ? result["salary"] : String.Empty,
                    SalaryRange = result.Fields.ContainsKey("salaryRange") ? result["salaryRange"] : String.Empty,
