...
namespace Escc.EastSussexGovUK.Umbraco.Forms
{
    ...
    [Authorize]
    public class RetentionApiController : UmbracoApiController
    {
        /// <summary>
        ...
        public void ApplySetDateRetentionSchedule()
        {
            try
            {
                LogHelper.Info<RetentionApiController>($"ApplyRetentionSchedules started at {DateTime.UtcNow} (UTC).");
                                ...
                                foreach (var field in record.RecordFields)
                                {
                                    if (field.Value.Alias == "deleteAfter" && field.Value.HasValue() && !String.IsNullOrEmpty(field.Value.ValuesAsString()))
                                    {
                                        ...
                                        if (DateTime.TryParse(field.Value.ValuesAsString(), out retentionDate))
                                        {
                                            ...
                                            {
                                                LogHelper.Info<RetentionApiController>($"Deleting record '{record.UniqueId}' for form '{form.Name}' with Id '{form.Id}' as its retention date '{field.Value.ValuesAsString()}' has passed.");
                ...
                LogHelper.Info<RetentionApiController>($"ApplyRetentionSchedules completed at {DateTime.UtcNow} (UTC).");
            }
            catch (Exception exception)
            {
                LogHelper.Error<RetentionApiController>(exception.Message, exception);
                throw;
            }
        }
    }
