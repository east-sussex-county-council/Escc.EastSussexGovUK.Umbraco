...
namespace Escc.EastSussexGovUK.Umbraco.Api.Jobs.Api
{
    ...
    public class PublicJobsController : BaseJobsApiController
    {
        ...
        public async Task<JobSearchResult> Jobs([FromUri] string baseUrl)
        {
            try
            {
                return await base.Jobs(JobsSet.PublicJobs, new JobSearchQueryConverter(ConfigurationManager.AppSettings["TranslateObsoleteJobTypes"]?.ToUpperInvariant() == "TRUE").ToQuery(HttpUtility.ParseQueryString(Request.RequestUri.Query)), new Uri(baseUrl));
            }
            catch (Exception e)
            {
        ...
        // GET /umbraco/api/publicjobs/salaryfrequencies/
        [HttpGet]
        public async Task<IList<JobsLookupValue>> SalaryFrequencies()
        {
            try
            {
                return await base.ReadSalaryFrequencies(JobsSet.PublicJobs);
            }
            catch (Exception e)
            {
                e.ToExceptionless().Submit();
        ...
        // GET /umbraco/api/publicjobs/paygrades/
        [HttpGet]
        public async Task<IList<JobsLookupValue>> PayGrades()
        {
            try
            {
                return await base.ReadPayGrades(JobsSet.PublicJobs);
            }
            catch (Exception e)
            {
                e.ToExceptionless().Submit();
