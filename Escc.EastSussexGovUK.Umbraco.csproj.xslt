...
>
  ...
  <xsl:template match="msbuild:Project/msbuild:ItemGroup/msbuild:Reference/msbuild:HintPath">
    <xsl:call-template name="UpdateHintPath">
      ...
      <xsl:with-param name="ref_7" select="'UmbracoExamine.Azure.dll'" />
      <xsl:with-param name="ref_8" select="'Examine.dll'" />
      <xsl:with-param name="ref_9" select="'UmbracoExamine.dll'" />
    </xsl:call-template>
  </xsl:template>
